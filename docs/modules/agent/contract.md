...
| `assets.content` | 模块 intake/parsing | 项目素材正文。文件/Git 资产应由 parsing 清洗后回写 |
| 用户自然语言消息 | 前端输入 | — |
...
- 当前 v1 主对话实现仍以 `drafts.html_content` 作为默认上下文。
- `assets.content` 是项目素材正文的统一来源，适用于后续素材读取工具或带素材增强的 Prompt 构建链路。
...
- 当前实现默认不会自动拼接项目素材正文；如需读取项目素材，优先使用 `assets.content`，而不是原始文件路径或文件内容
...
- 模块 parsing / intake 在 `assets.content` 中提供可直接消费的素材正文

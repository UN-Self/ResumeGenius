...
- 每个面板头部有折叠/展开按钮
- 折叠时：`grid-template-columns: 0 minmax(300px,5fr) minmax(220px,2fr)` 或 `minmax(200px,2fr) minmax(300px,5fr) 0` 或 `0 1fr 0`
- 过渡动画：`transition: grid-template-columns 300ms ease-in-out`
- 折叠状态存在组件 state 中，不持久化

### 响应式断点

> 当前实现使用比例 Grid 单位（`minmax + fr`），面板默认全部展开，由用户手动折叠。

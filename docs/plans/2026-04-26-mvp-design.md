...
| 成员 | 模块 | 核心职责 |
|---|---|---|
| 人1 | D — TipTap 编辑器 | 所见即所得编辑、A4 画布、自动保存 |
| 人2 | E — 版本管理 + PDF 导出 | 版本快照、chromedp PDF 导出 |
| 人3 | B — 文件解析 + AI 初稿 | PDF 解析、GLM API 调用、HTML 初稿生成 |
| 人4 | C — AI 对话助手 | SSE 流式对话、HTML 提取、应用到简历 |
| 人5 | A — 项目管理 + 文件上传 | 项目 CRUD、文件上传、资产管理 |
...
包含：PDF 文本提取、调用 GLM API 生成 HTML 初稿、返回 HTML 写入 drafts 表并自动创建 version
...
```
A: POST /api/v1/intake/projects              → 创建 project
A: POST /api/v1/intake/projects/:id/assets   → 上传文件，返回 asset_id
B: POST /api/v1/parsing/parse                → 输入 asset_id，输出 draft_id
D: GET  /api/v1/workbench/drafts/:id         → 返回 HTML
D: PUT  /api/v1/workbench/drafts/:id         → 保存 HTML
C: POST /api/v1/agent/sessions               → 创建会话（绑定 draft_id）
C: POST /api/v1/agent/sessions/:id/chat      → SSE 流式对话
E: GET  /api/v1/render/versions?draft_id=    → 版本列表
E: POST /api/v1/render/versions              → 创建快照
E: POST /api/v1/render/export                → 触发 PDF 导出
E: GET  /api/v1/render/export/:task_id       → 查询导出状态
```
...
```
/                    → 项目列表（模块 A）
/projects/new        → 创建项目 + 上传文件（模块 A + B）
/editor/:projectId   → 编辑器页面（模块 D 主页面）
                       左侧: A4 画布（TipTap）
                       右侧: AI 面板（模块 C，可收起）
                       顶部: 操作栏（保存/版本/导出 → 模块 E）
```
...
| Day 8 | D+E 集成 | 编辑器能保存 + 导出 PDF |
| Day 10 | A+B+D 集成 | 上传文件 → 解析 → 编辑 |
| Day 12 | A+B+C+D+E 全链路 | 完整用户流程 |

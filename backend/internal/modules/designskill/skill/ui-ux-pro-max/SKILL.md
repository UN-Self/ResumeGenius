...
In ResumeGenius this skill is embedded in the Go backend. Use the shared Go searcher instead of the original Python helper:

```bash
cd backend
go run ./cmd/uxsearch "<keyword>" --domain <domain> --max 3
go run ./cmd/uxsearch "<keyword>" --stack react --json
```
...
Use the Go searcher multiple times to gather comprehensive information. Search until you have enough context.

```bash
go run ./cmd/uxsearch "<keyword>" --domain <domain> --max <max_results>
```
...
```bash
go run ./cmd/uxsearch "<keyword>" --stack html-tailwind
```
...
go run ./cmd/uxsearch "beauty spa wellness service" --domain product

# 2. Search style (based on industry: beauty, elegant)
go run ./cmd/uxsearch "elegant minimal soft" --domain style

# 3. Search typography
go run ./cmd/uxsearch "elegant luxury" --domain typography

# 4. Search color palette
go run ./cmd/uxsearch "beauty spa wellness" --domain color

# 5. Search landing page structure
go run ./cmd/uxsearch "hero-centric social-proof" --domain landing

# 6. Search UX guidelines
go run ./cmd/uxsearch "animation" --domain ux
go run ./cmd/uxsearch "accessibility" --domain ux

# 7. Search stack guidelines (default: html-tailwind)
go run ./cmd/uxsearch "layout responsive" --stack html-tailwind
```
